...
and/or address bar primary selection behavior of Firefox. Make sure Firefox is up-to-date and closed.'
...
show_usage(){
  echo "Usage: ${BASH_SOURCE[0]} --fix [FIX_NAME] [OPTION]...
FIX_NAME : clickselect or primaryselect
...
show_help(){
  echo "Usage: ${BASH_SOURCE[0]} --fix [FIX_NAME] [OPTION]...
Disable broken clickSelectsAll behavior and/or address bar primary selection behavior in your Firefox installation.
...
OPTIONs:
  ...
  -x, --fix  FIX_NAME      Fix to apply (can be used multiple times)
                              Fixes available : clickselect, primaryselect
...
Examples:
  ...
  # Fix the clickSelectsAll behavior only.
  ${BASH_SOURCE[0]} -x clickselect

  # Fix the clickSelectsAll behavior and primary selection behavior.
  ${BASH_SOURCE[0]} -x clickselect -x primaryselect
...
greet_and_apply_options(){
  ...
  if [[ ! "${options[fixes]}" ]]; then
    echo "Fix name missing"
    show_usage
    terminate '2'
  fi
...
fix_primaryselect(){
  edit_file 'selection' 's#!this.window.windowUtils.isHandlingUserInput ||#//!this.window.windowUtils.isHandlingUserInput ||#' "${unpack_dir}/modules/UrlbarInput.jsm"
}
...
fix_firefox(){
  ...
  for fix in $(echo ${options[fixes]}); do
    case $fix in
      "clickselect")
        echo "-> Fixing clickSelectsAll behavior…"
        fix_clickselect
      ;;
      "primaryselect")
        echo "-> Fixing address bar primary selection behavior…"
        fix_primaryselect
      ;;
      "" | *)
        echo "Incorrect or empty fix name : $fix"
        terminate '2'
      ;;
    esac
  done
